...
test.describe('Login Tests', () => {
  ...
  test('Should not log in with invalid credentials', async ({ page }) => {
    await navigateToHomePage(page);
    await login(page, 'invaliduser@example.com', 'wrongpassword');
    
    // Wait for the error message to be visible
    const errorMessageSelector = 'text="We couldn’t find an account matching the username and password you entered. Please check your username and password and try again."'; // Adjust selector as needed
    const errorMessageLocator = page.locator(errorMessageSelector);
    
    await expect(errorMessageLocator).toBeVisible();
    
    // Optionally, verify the exact error message
    const errorMessage = await errorMessageLocator.textContent();
    expect(errorMessage).toContain('We couldn’t find an account matching the username and password you entered. Please check your username and password and try again."');
  });
});
